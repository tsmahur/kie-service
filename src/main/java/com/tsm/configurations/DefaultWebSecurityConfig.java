...
import com.tsm.configurations.AuthTokenFilter;
...
public class DefaultWebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure (HttpSecurity http) throws Exception {
        http.cors().and()
                .csrf().disable()
                //url that do not need authentication
    ...
    public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception {
        ...
        //user name and pwd can be config via ldap / or user jwt token
    }
...
/*    @Bean
    ...
    CorsConfigurationSource corsConfigurationSource() {
        ...
        corsConfiguration.setAllowCredentials(true);
        corsConfiguration.setAllowedMethods (Arrays.asList(HttpMethod.GET.name(), HttpMethod.HEAD.name(), HttpMethod.POST.name(), HttpMethod.DELETE.name(), HttpMethod.PUT.name()));
        corsConfiguration.applyPermitDefaultValues();
        source.registerCorsConfiguration ("/**", corsConfiguration);
        return source;
    }
